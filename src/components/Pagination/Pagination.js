...
const PaginationItem = styled.div`
    width: 32px;
    margin: 0 5px;
    background-color: ${({active, theme}) => active ? theme.mainBlue : 'white' };
...
const calculatePaginationSetup = (activePage, totalPosts, postsPerPage) => {
    ...
    if(totalPagesAmount < 7) return Array.from({ length: totalPagesAmount }, (_, i) => i + 1);

    if(activePage <= 4) return [1,2,3,4,5,'...', totalPagesAmount];

    if(totalPagesAmount-activePage <=3) return [1, '...', totalPagesAmount-4, totalPagesAmount -3, totalPagesAmount-2, totalPagesAmount-1, totalPagesAmount]

    return [1, '...', activePage-1, activePage, activePage+1, '...', totalPagesAmount];
}
const Pagination = ({ postsPerPage, handlePostsAmountChange, currentActivePage, totalPosts, changeActivePage, moveToNextPage, moveToPreviousPage }) => {
    ...
    return (
      <PaginationWrapper>
        <LeftSideWrapper>
          ...
          <DropdownWrapper>
            <Dropdown
              ...
              indexOfDefaultValue={dropdownOptions.findIndex(
                (option) => option.value === postsPerPage
              )}
        ...
        <RightSideWrapper>
          ...
          />
          {itemsToBeDisplayedInPagination.map((arrItem) => (
            <PaginationItem
              active={currentActivePage === arrItem}
              onClick={() => arrItem === '...' ? null : changeActivePage(arrItem)}
            >
              {arrItem}
            </PaginationItem>
          ))}
