...
const SuccessWindow = ({ className, successMessage }) => {
  return (
    <StyledSuccessWindow className={className}>
      <motion.span className="message" initial={{y: -1000}} animate={{y:0}} transition={{duration: 1, type: "tween"}}>{successMessage}</motion.span>
      <motion.svg
      ...
      <Button className="btn-success" title="Sign in" onClick={onClick} variants={buttonVariants}/>
    </StyledSuccessWindow>
  );
};
